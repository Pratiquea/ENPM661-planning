...
#Define A Star
def path_planning(nStart,nGoal,robot_radius,grid,ox,oy,algo_flag,res,show_animation):
    ...
    pt2 = ax.plot(nStart.x,nStart.y,".g")[0]
    while 1:
        ...
        if show_animation:
            if current != nStart:
                ...
                pt1.set_data(current.x,current.y)
                ax.draw_artist(pt1)
        ...
        for i,_ in enumerate(motion):
            ...
            if new_id in active:
                if active[new_id].c2c > new_node.c2c:
                    ...
                    if algo_flag is "astar":
                        heapq.heappush(cost_heap,[active[new_id].c2c+calc_heuristic(nGoal,active[new_id]),active[new_id]])
                    elif algo_flag is "dijk":
                ...
                if algo_flag =="astar":
                    ...
                    heapq.heappush(cost_heap,[active[new_id].c2c,active[new_id]])
                if show_animation:
                    pt2.set_data(active[new_id].x,active[new_id].y)
                    ax.draw_artist(pt2)
                    fig.canvas.blit(ax.bbox)
...
def generate_map(robot_radius,res,clearence):
    ...
    xx = res*xx
    yy = res*yy
    robot_radius = clearence + robot_radius/res
    #Grid Boundary
    o1 = xx
    o2 = yy
    o3 = xx
    ...
    grid = np.full(((150/res) +2, (250/res) +2), True, dtype=bool)
    ...
    for i in range((150/res) +2):
        for j in range((250/res) +2):
            if grid[i][j]==False:
                ox.append(j)
                oy.append(i)
...
def main():
    ...
    nStart=Node((sx/res),(sy/res),0.0,-1)
    ...
    nGoal = Node(gx/res,gy/res,0.0,-1)
    ...
    if start==True and goal==True:
        start = time.time()
        rx,ry,dead = path_planning(nStart,nGoal,robot_radius=robot_radius,grid=grid,ox=ox,oy=oy,algo_flag=algo_flag,res=res,show_animation=show_animation)
        end = time.time()
